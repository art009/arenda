...
_.merge = function(o1, o2) {
  ...
  for (var key in o2) {
		if (o2[key] !== null && o2.hasOwnProperty(key)) {
			try {
				o1[key] = (o2[key].constructor === Object) ? _.merge(o1[key], o2[key]) : o2[key];
			} catch(e) {
				o1[key] = o2[key];
			}
		}
...
_.getByClass = function(wrap, className) {
  ...
  for (var i = 0; i < el.length; i++) {
		if (el[i].className.match(className)) {
      arr.push(el[i]);
    }
	}
	return arr;
...
_.getStyle = function(el, style, number) {
	switch(style) {
		...
		default :
      var obj = (typeof el.currentStyle !== 'undefined') ? el.currentStyle : document.defaultView.getComputedStyle(el, null);

      if (obj[style]) {
				return (number) ? parseFloat(obj[style].toString().replace(/(pt|px|%)/g, '')) : obj[style];
			} else {
				return 0;
			}
		break;
...
_.reduceText = function(str, length, points) {
	return (str.length > length) ? str.slice(0, length) + ((points)? '...' : '') : str;
};
